...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		init: function () {
			...
			if( $c.isEmptyObject(that.defaultImg)){ that.defaultImg = that.obj.find('img'); }
			
			that.createImgUploadControls();
			
			if( $c.isEmptyObject(that.options.loadPicture)){				
		...
		createImgUploadControls: function(){
			...
			if( $c.isEmptyObject(that.croppedImg)){ cropControlRemoveCroppedImage=''; }
			if( !$c.isEmptyObject(that.options.loadPicture)){ cropControlUpload='';}
			...
			else{	that.imgUploadControl = $c('#'+that.options.customUploadButtonId); that.imgUploadControl.show();	}

			if( !$c.isEmptyObject(that.croppedImg)){
		...
		bindImgUploadControl: function(){
			...
			if( !$c.isEmptyObject(that.croppedImg)){
				...
				that.cropControlRemoveCroppedImage.on('click',function(){ 
					...
					if( !$c.isEmptyObject(that.defaultImg)){ 
						that.obj.append(that.defaultImg);
					}
					
					if(that.options.outputUrlId !== ''){	$c('#'+that.options.outputUrlId).val('');	}
				    ...
				    if (typeof FileReader == "undefined"){
						...
						reader.onload = function (e) {
							...
							image.onload = function(){
								...
								if( !$c.isEmptyObject(that.croppedImg)){ that.croppedImg.remove(); }
		...
		loadExistingImage: function(){
			...
			if( $c.isEmptyObject(that.croppedImg)){
				...
				var img =$c('<img src="'+ that.options.loadPicture +'">');
				that.obj.append(img);
				img.load(function() {
		...
		afterUpload: function(data){
            ...
            if (response.status == 'success') {
                ...
                var img = $c('<img src="'+response.url+'">')
		...
		createModal: function(){
			...
			$c('body').append(modalHTML);
			
			that.modal = $c('#croppicModal');
			
			that.obj = $c('#croppicModalObj');
			...
			that.img.on("mousedown touchstart", function(e) {
				...
				that.img.css('z-index', 1000).on("mousemove touchmove", function(e) {
					...
					that.img.offset({
						...
						$c(this).removeClass('draggable').css('z-index', z_idx);
					});
		...
		afterCrop: function (data) {
            ...
            if (response.status == 'success') {
                ...
                if (that.options.outputUrlId !== '') { $c('#' + that.options.outputUrlId).val(response.url); }
		...
		reset:function(){
			...
			if( !$c.isEmptyObject(that.croppedImg)){ 
				...
				if(that.options.outputUrlId !== ''){	$c('#'+that.options.outputUrlId).val(that.croppedImg.attr('url'));	}
			}
		...
		destroy:function(){
			var that = this;
			if(that.options.modal && !$c.isEmptyObject(that.modal) ){ that.destroyModal(); }
			if(that.options.imgEyecandy && !$c.isEmptyObject(that.imgEyecandy) ){  that.destroyEyecandy(); }
			if( !$c.isEmptyObject( that.cropControlsUpload ) ){  that.cropControlsUpload.remove(); }
			if( !$c.isEmptyObject( that.cropControlsCrop ) ){   that.cropControlsCrop.remove(); }
			if( !$c.isEmptyObject( that.loader ) ){   that.loader.remove(); }
			if( !$c.isEmptyObject( that.form ) ){   that.form.remove(); }
			that.obj.html('');
		},
        ...
        CreateFallbackIframe: function () {
            ...
            if (!that.isAjaxUploadSupported()) { 
                ...
                var myContent = '<!DOCTYPE html>'
                                ...
                                + $c("#" + that.id + '_imgUploadField')[0].outerHTML
                                + '</form></body></html>';
                ...
                that.iframeobj = $c("#" + that.id + "_upload_iframe");                
