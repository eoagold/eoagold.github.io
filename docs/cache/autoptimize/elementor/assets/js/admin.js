...
(function ($) {
  var ElementorAdmin = elementorModules.ViewModule.extend({
    ...
    bindEvents: function bindEvents() {
      ...
      $('#elementor-landingpress-library-sync-button').on('click', function (event) {
        event.preventDefault();
        var $thisButton = $(this);
        $thisButton.removeClass('success').addClass('loading');
        $.post(ajaxurl, {
          action: 'elementor_reset_landingpress_library',
          _nonce: $thisButton.data('nonce')
        }).done(function () {
          $thisButton.removeClass('loading').addClass('success');
        });
    ...
    initTemplatesImport: function initTemplatesImport() {
      if (!elementorCommon.elements.$body.hasClass('edit-php')) {
        return;
      }
    ...
    initMaintenanceMode: function initMaintenanceMode() {
      var MaintenanceMode = __webpack_require__(518);
